...
def test_promedio_de_facturación_por_estación(datos:List[ITV])->None:
    print("\ntest_promedio_de_facturación_por_estación")
    f_i = date(2024,1,1)
    promedio = promedio_de_facturación_por_estación(datos,f_i)
    print(f"La facturación desde el {f_i} es: {promedio}")
    f_f = date(2019,10,31)
    promedio = promedio_de_facturación_por_estación(datos,f1=f_f)
    print(f"\nLa facturación hasta el {f_f} es: {promedio}")
    f_i = date(2023,6,1)
    f_f = date(2023,9,30)
    promedio = promedio_de_facturación_por_estación(datos,f_i,f_f)
    print(f"\nLa facturación entre el {f_i} y el {f_f} es: {promedio}")

def test_n_matrículas_más_importe_por_tipo(datos:List[ITV])->None:
    print("\ntest_n_matrículas_más_importe_por_tipo")
    print(f"Las matrículas tres matrículas son:")
    for c,v in n_matrículas_más_importe_por_tipo(datos).items():
        print(f"{c} --> {v}")

    print(f"\nLas matrículas cuatro matrículas son:")
    for c,v in n_matrículas_más_importe_por_tipo(datos,n=4).items():
        print(f"{c} --> {v}")

def test_tipo_más_facturación_favorable(datos:List[ITV])->None:
    print("\ntest_tipo_más_facturación_favorable")
    promedios = tipo_más_facturación_favorable(datos)
    tipo_max = max(promedios.items(),key = lambda a: a[1])
    print(f"El tipo que más facturación con inspecciones favorables es: {tipo_max}")
...
if __name__ == "__main__":
    ...
    test_promedio_de_facturación_por_estación(ITVs)
    test_n_matrículas_más_importe_por_tipo(ITVs)
    test_tipo_más_facturación_favorable(ITVs)