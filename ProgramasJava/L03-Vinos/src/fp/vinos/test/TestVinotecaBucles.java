...
public class TestVinotecaBucles {
	...
	private static void mostrarColeccion(Collection<Vino> vinos) {
		vinos.stream().forEach(v -> System.out.println("\t" + v));
	}
	...
	public static void main(String[] args) {
		...
		Vinoteca vinos2 = FactoriaVinos.leerVinoteca("data/wine_reviews.csv","B");
	...
	public static void testCalcularNumeroVinosPais(Vinoteca vinos, String pais) {
		...
		try {
			System.out.println(String.format("\tHay %d  vinos de %s: ", (vinos).calcularNumeroVinosDePais(pais), pais));
		} catch (Exception e) {
			System.out.println("Excepción inesperada capturada:\n   " + e);
		}
	}
	...
	public static void testCalcularNumeroVinosDePaisConPuntuacionSuperior(Vinoteca vinos, String pais,
		...
		try {
			System.out.println(String.format("\tHay %d  vinos del pais %s con puntuación superior a %d: ",
					vinos.calcularNumeroVinosDePaisConPuntuacionSuperior(pais, umbralPuntuacion), pais,
					umbralPuntuacion));
		} catch (Exception e) {
			System.out.println("Excepción inesperada capturada:\n   " + e);
		}
	...
	private static void testObtenerVinosBaratos(Vinoteca vinos, Double precio) {
		...
		try {
			String msg = String.format("\tLos vinos del con precio inferior a %.2f son", precio);
			System.out.println(msg);
	...
	private static void testExisteVinoDeUvaEnRegion(Vinoteca vinos, String uva, String region) {
		...
		try {
			String msg = String.format("\t¿Hay algún vino de la uva %s en la región %s? %s", uva, region,
					vinos.existeVinoDeUvaEnRegion(uva, region));
			System.out.println(msg);
		} catch (Exception e) {
			System.out.println("Excepción inesperada capturada:\n   " + e);
		}
	...
	public static void testCalcularUvasDeRegion(Vinoteca vinos, String region) {
		...
		try {
			...
			String msg = String.format("\tHay %d uvas de la región %s y son", uvas.size(), region);
			System.out.println(msg);
			uvas.stream().forEach(uva -> System.out.println("\t" + uva));
		} catch (Exception e) {
			System.out.println("Excepción inesperada capturada:\n   " + e);
		}
	}
	...
	private static void testCalcularTotalPuntosVinosDeRegion(Vinoteca vinos, String region) {
		...
		try {
			String msg = String.format("\tEl total de puntos de la región %s es %d", region,
					vinos.calcularTotalPuntosVinosDeRegion(region));
			System.out.println(msg);
		} catch (Exception e) {
			System.out.println("Excepción inesperada capturada:\n   " + e);
		}
	...
	private static void testCalcularMediaPuntosVinosDeUva(Vinoteca vinos, String uva) {
		...
		try {
			String msg = String.format("\tLa media de puntos de los vinos de la uva %s es %.2f", uva,
					vinos.calcularMediaPuntosVinosDeUva(uva));
			System.out.println(msg);
		} catch (Exception e) {
			System.out.println("Excepción inesperada capturada:\n   " + e);
		}
	...
	private static void testObtenerVinoMejorPuntuadoDePais(Vinoteca vinos, String pais) {
		...
		try {
			System.out.println(String.format("\tEl vino mejor puntuado del pais %s es: %s", pais,
					vinos.obtenerVinoMejorPuntuadoDePais(pais)));
		} catch (Exception e) {
			System.out.println("Excepción inesperada capturada:\n   " + e);
		}
